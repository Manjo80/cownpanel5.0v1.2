...
Panel keine Tastatur hat. Es gibt seit Version `2026-07-29.27` nur noch
...
- **APP ERSTELLEN** — `CreateApplication(CUSTOM_AID, true)` auf
  ...
  Version `2026-07-29.24`/`.26` gab es zusätzlich eine zweite,
  klassische 2K3DES-Test-App ("APP ERSTELLEN (AES)" hieß der AES-Button
  damals, zur Unterscheidung) -- seit `2026-07-29.27` endgültig entfernt
  (siehe Abschnitt 15), jetzt gibt es nur noch diese eine App.
...
- **App-Auswahl: fest verdrahtete AID statt "aktive"/zuletzt gewählte
  App.** Bis Version `2026-07-29.24`/`.26` zeigte `activeCustomAid` in
  dieser Test-Firmware bewusst auf die zuletzt per "APP ERSTELLEN"/"APP
  ERSTELLEN (AES)" gewählte App (praktisch, um zwei parallele
  Cipher-Testpfade zu halten -- siehe Abschnitt 15, Testsequenz-Funde
  zur Verwechslungsgefahr genau dadurch). Seit `2026-07-29.27` gibt es
...
`2026-07-29.27` nur noch 9 statt 15 Schritte -- siehe "Entschieden:
2K3DES-Test-App endgültig entfernt" unten).** Auf Nutzerwunsch, nachdem
manuelles Testen in falscher Reihenfolge zu einem schwer einzuordnenden
Log geführt hatte ("Idealerweise machst du mir einen Testbutton, der
alles nacheinander testet, wie es gebraucht wird, mit Rückfragen Karte
...
### Entschieden: 2K3DES-Test-App endgültig entfernt, nur noch AES (Version `2026-07-29.27`)
...
Fix weiter oben). Hin und her: erst entfernt (`.25`), dann kurz wieder
zurückgeholt (`.26`), weil ein Test mit zwei Terminal-Boards danach
aussah, als würde die zweite (2K3DES-)App noch gebraucht -- stellte
sich aber als reine Verwechslung heraus: auf einem Board wurde die
AES-App angelegt, auf dem zweiten wurde versehentlich die (nicht mehr
vorhandene) 2K3DES-App erwartet. Endgültige Entscheidung des Testers:
**nur noch AES, die 2K3DES-Test-App ergibt keinen Sinn und bleibt
draußen.** Es gibt jetzt nur noch EINE Test-App (`CUSTOM_AID`, AID
0x654321, IMMER mit AES-Schlüsseln angelegt), der Button heißt schlicht
"APP ERSTELLEN". Die `activeCustomAid`-Indirektion samt der "Ziel:
aktive Test-App"-Hinweiszeile ist entfernt, da das Verwechslungsrisiko
strukturell wegfällt (nur noch eine App). TEST-SEQUENZ schrumpft von 15
auf 9 Schritte (nur noch ein Anlegen/ChangeKey/Credit/Debit/GetValue-
Zyklus statt zwei).
...
(`desfireDeepRead()`/KARTEN INFO). Entfernt ist ausschließlich der Pfad,
der die EIGENE Test-App zwingend als 2K3DES anlegte.

**Für Mehrgeräte-Tests:** beide Terminals müssen auf denselben
Firmware-Stand geflasht werden, sonst zeigt eines noch den alten
2K3DES-Button/die alte AID (0x123456) und das andere nicht mehr --
genau das hatte die kurzzeitige Verwirrung ausgelöst.
